(function (d3) {
  ...
  d3.json('/get_max_count', function(data) {
  ...
  const sliderTooltip = d3.select('#sliderTooltip');

  let selectedDateString;

  d3.json('/get_date_range', function(data) {
      ...
      let title = d3.select('#bar-chart-title');
      ...
      dateSlider
          ...
          function updateDateLabels() {
              ...
              d3.select('.value-min').text(new Date(startDate).toLocaleDateString('en-US', {
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric'
              })).style('font-size', '12px').style('font-weight', 'bold');
              d3.select('.value-max').text(new Date(endDate).toLocaleDateString('en-US', {
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric'
              })).style('font-size', '12px').style('font-weight', 'bold');
  ...
  d3.json('get_geojson', function(data) {
  ...
  function createColorLegend() {
      ...
      const colorLegendContainer = d3.select('#color-legend-container');
  ...
  function loadData() {
      d3.json('/get_geojson', function(geoData) {
          ...
          d3.json(`/get_news_count?credibility=${credibilityQuery}&date=${selectedDateString}`, function(sentimentResponse) {
  ...
  d3.json('/get_most_negative_provs', function(barChartData) {
  ...
  function createBarChart(data) {
      ...
      const x = d3.scaleLinear().range([0, widthN]);
      const y = d3.scaleBand().range([0, heightN]).padding(0.1);
      const barColorScale = d3.scaleOrdinal(d3.schemeTableau10 );
  ...
  //PIE CHART
  d3.json('/get_total_sentiment', function(data) {
      createPieChart(data);
  });

  function createPieChart(data) {
      const pieSvg = d3.select('#pie-container')
          .append('svg')
          .attr('width', 350)
          .attr("height", heightN + margin.top + margin.bottom)
          .attr('id', 'pie-svg');
      ...
      const pieColorScale = d3.scaleOrdinal(d3.schemeTableau10 );
      const pieC = d3.pie().value(d => d.count).sort(null);
      const arcC = d3.arc().outerRadius(radius * 0.9).innerRadius(0);
  ...
  //LINE CHART
  d3.json('/get_sentiment_count_each_date', function(data) {
      ...
      const svgLine = d3.select('#line-container')
          .append('svg')
              .attr('width', 365)
              .attr('height', heightN + margin.top + margin.bottom)
          .append('g')
      ...
      listeningRect.on('mousemove', function () {
          const xCoord = d3.event.clientX - this.getBoundingClientRect().left;
          const bisectDate = d3.bisector(d => d.year_month).left;
